...
C_SOURCES = \
  ...
  Src/usart.c  
ASM_SOURCES = \
  SW4STM32/startup_stm32f207xx.s
...
# macros for gcc
AS_DEFS =
C_DEFS =  -D__weak="__attribute__((weak))" -D__packed="__attribute__((__packed__))" -DUSE_HAL_DRIVER -DSTM32F207xx
# includes for gcc
AS_INCLUDES =
C_INCLUDES = -IInc
C_INCLUDES += -IDrivers/STM32F2xx_HAL_Driver/Inc
C_INCLUDES += -IDrivers/STM32F2xx_HAL_Driver/Inc/Legacy
C_INCLUDES += -IDrivers/CMSIS/Device/ST/STM32F2xx/Include
...
CFLAGS = -mthumb -mcpu=cortex-m3 -fno-aggressive-loop-optimizations $(C_DEFS) $(C_INCLUDES) $(OPT) $(WFLAGS)
...
#########################################################
# link script
LDSCRIPT = "SW4STM32/IntelliRoast-Embedded/STM32F207ZGTx_FLASH.ld"
# libraries
LIBS = -lc -lm -lnosys
LIBS += 
LIBDIR =
...
LDFLAGS = -mthumb -mcpu=cortex-m3 $(LDSPECS) -T$(LDSCRIPT) $(LIBDIR) $(LIBS) -Wl,-Map=$(BUILD_DIR)/$(TARGET).map,--cref -Wl,--gc-sections
